# Interactions here don't have target popups because the target is always the user itself.
# Why would you do this?
interaction-PinchSelf-name = Щипнуть себя
interaction-PinchSelf-description = Говорят, это помогает убедиться, что окружающий вас Ад реален.
interaction-PinchSelf-success-self-popup = Вы щипаете себя... Ай!
interaction-PinchSelf-success-others-popup = { THE($user) } щипает { REFLEXIVE($user) }... Выглядело больно!
interaction-PinchSelf-fail-self-popup = Вам не удалось ущипнуть себя. Вам повезло.
interaction-PinchSelf-delayed-self-popup = Вы щипаете себя...
interaction-PinchSelf-message-1 = Ауч!
interaction-PinchSelf-message-2 = Ай!
interaction-PinchSelf-message-3 = Уф!
interaction-MakeSleepSelf-name = Уснуть
interaction-MakeSleepSelf-description = Уложить себя спать.
interaction-MakeSleepSelf-delayed-self-popup = Вы пытаетесь уснуть...
interaction-MakeSleepSelf-fail-self-popup = Вы не можете сейчас уснуть.
interaction-MakeSleepSelf-success-self-popup = Вы засыпаете.
interaction-MakeSleepSelf-success-others-popup = { THE($user) } засыпает.
